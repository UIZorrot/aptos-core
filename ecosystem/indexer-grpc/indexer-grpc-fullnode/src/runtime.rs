...
use aptos_protos::internal::fullnode::v1::{
    ...
    get_transactions_from_node_response,
    stream_status::StatusType,
    GetTransactionsFromNodeRequest, GetTransactionsFromNodeResponse, StreamStatus,
};
...
type ResponseStream =
    Pin<Box<dyn Stream<Item = Result<GetTransactionsFromNodeResponse, Status>> + Send>>;
...
pub fn get_status(
    ...
    ledger_chain_id: u8,
) -> GetTransactionsFromNodeResponse {
    GetTransactionsFromNodeResponse {
        response: Some(get_transactions_from_node_response::Response::Status(
            StreamStatus {
