...
fn test_vote() {
    ...
    // Cannot create partial voting proposal before enable the feature flag.
    assert!(!is_partial_voting_proposal(&mut harness, proposal_id));
    ...
    // Voters cannot vote on a non partial voting proposal twice even after the feature flag is enabled.
    assert_abort!(
        partial_vote(
            &mut harness,
            &validator_1,
    ...
    assert!(is_partial_voting_proposal(&mut harness, proposal_id));
    assert_eq!(
        get_voting_power_usage(&mut harness, validator_1_address, proposal_id),
        0
    );
    assert_eq!(
        get_voting_power_usage(&mut harness, validator_2_address, proposal_id),
        0
    );
