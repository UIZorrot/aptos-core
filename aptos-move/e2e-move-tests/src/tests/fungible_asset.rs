...
fn test_basic_fungible_token() {
    ...
    let result = h.run_entry_function(
        &alice,
        str::parse(&format!("0x{}::managed_fungible_token::mint", *alice.address())).unwrap(),
        vec![],
        vec![
    ...
    let result = h.run_entry_function(
        &alice,
        str::parse(&format!("0x{}::managed_fungible_token::transfer", *alice.address())).unwrap(),
        vec![],
        vec![
            bcs::to_bytes(alice.address()).unwrap(),
    ...
    let result = h.run_entry_function(
        &alice,
        str::parse(&format!("0x{}::managed_fungible_token::burn", *alice.address())).unwrap(),
        vec![],
        vec![
            bcs::to_bytes(bob.address()).unwrap(),
    ...
    let token_addr = account_address::create_token_address(*alice.address(), "test collection name", "test token name");
    let alice_primary_store_addr = account_address::create_derived_object_address(*alice.address(), token_addr);
    let bob_primary_store_addr = account_address::create_derived_object_address(*bob.address(), token_addr);
    ...
    let mut alice_store: FungibleStore = h
        .read_resource_from_resource_group(&alice_primary_store_addr, obj_group_tag.clone(), fungible_store_tag.clone())
        .unwrap();

    let bob_store: FungibleStore = h
        .read_resource_from_resource_group(&bob_primary_store_addr, obj_group_tag, fungible_store_tag)
        .unwrap();
