...
pub fn is_partial_voting_proposal(harness: &mut MoveHarness, proposal_id: u64) -> bool {
    let fun = MemberId::from_str("0x1::aptos_governance::is_partial_voting_proposal").unwrap();
    let res = harness
        .execute_view_function(fun, vec![], vec![bcs::to_bytes(&proposal_id).unwrap()])
        .unwrap();
    bcs::from_bytes::<bool>(&res[0]).unwrap()
}

pub fn get_voting_power_usage(
    harness: &mut MoveHarness,
    stake_pool: AccountAddress,
    proposal_id: u64,
) -> u64 {
    let fun = MemberId::from_str("0x1::aptos_governance::get_voting_power_usage").unwrap();
    let res = harness
